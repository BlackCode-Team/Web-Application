...
//////line chart 
$currentHour = (int) date('H'); // get current hour as integer

$vehicles = $vehiculeRepository->findAll();

$disponibles = []; // array to store available vehicles
$reserves = []; // array to store reserved vehicles

foreach ($vehicles as $vehicle) {
    // check if vehicle is available based on current time and status
    if ($currentHour >= 9 && $currentHour < 15 && $vehicle->getStatus() === 'disponible') {
        $disponibles[] = $vehicle;
    } else {
        $reserves[] = $vehicle;
    }
    ...
    'datasets' => [
        [
            ...
            'data' => [
                ...
                0, // set to 0 for last hour (15:00)
            ],
